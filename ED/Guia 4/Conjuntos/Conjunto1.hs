...
addS x (MkSet xs) = if pertenece x xs
								--then error "El conjunto ya contiene al elemento." --Si agrego este error no puedo utilizar la estructura para pasar de una lista a un conjunto. 
								then MkSet xs
								else MkSet (x : xs)
...
--En versiones sin el invariante, tambien se podria hacer como: contarNoRepetidos xs -- O sino se podria hacer: longitud (sinRepetidos xs) 
...
unionS (MkSet xs) (MkSet ys) = MkSet (sinRepetidos (xs ++ ys))
-- Si union la hiciera como MkSet (xs ++ ys)...

--G:
setTolist :: Eq a => Set a -> [a]
setTolist (MkSet xs) = xs
-- ... entonces sin repetidos deberia ser sinRepetidos xs
